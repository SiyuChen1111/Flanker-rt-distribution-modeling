...
DEFAULT_ROOT = RESULTS_ROOT / "repro_legacy_interim" / "true_single_subject_feasibility_accumrnn"
EVIDENCE_ROOT = Path(".sisyphus") / "evidence"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Analyze recurrent-accumulator single-subject feasibility outputs and write scorecard, verdict, and summary."
    )
            ...
            rows.append(
                {
                    ...
                    "user_id": str(summary.get("user_id", uid)),
...
def main() -> None:
    ...
    ww_path = RESULTS_ROOT / "repro_legacy_interim" / "true_single_subject_feasibility" / "reaggregated" / "feasibility_verdict.json"
    ...
    ww_verdict = _load_json(ww_path).get("verdict") if ww_path.exists() else "UNKNOWN"
    ...
    verdict = (
        "VGGACCUM-SINGLE-SUBJECT-FEASIBLE"
        if feasible_subjects >= max(1, total_subjects // 2 + total_subjects % 2) and all(v >= 1 for v in age_support.values())
        else "VGGACCUM-SINGLE-SUBJECT-NOT-FEASIBLE"
    )
    keep_recommendation = (
        "keep_investigating_accumrnn"
        if verdict == "VGGACCUM-SINGLE-SUBJECT-FEASIBLE"
        else "deprioritize_accumrnn"
    )
    ...
    ev5 = _write_evidence(5, "feasibility-scorecard-accumrnn", {"scorecard": _safe_rel(scorecard_path), "verdict": _safe_rel(verdict_path)})
    ...
    summary_lines = [
        ...
        f"- Previous VGG + WW single-subject verdict: `{ww_verdict}`",
    ...
    ev6 = _write_evidence(6, "framework-summary-accumrnn", {"summary": _safe_rel(summary_path), "verdict": verdict, "recommendation": keep_recommendation})
