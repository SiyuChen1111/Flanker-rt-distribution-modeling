"""True single-subject feasibility workflow for VGG + WW.

Definition (this branch):
  - Fit Stage-2 (WW) parameters using *one subject's own trials* (train split)
  - Evaluate on held-out trials from the *same subject*

This is a framework feasibility test (keep/drop VGG+WW), not a mechanism probe.

Modes:
  - audit-baseline: verify required inputs exist, write panel_manifest.json
  - build-panel: select bounded subject panel + write deterministic within-subject splits
  - fit: run per-subject Stage-2 fitting + evaluation for one age group
...
def parse_args() -> argparse.Namespace:
    ...
    # Fit config
    # Defaults are intentionally bounded so the panel workflow completes in reasonable time.
...
def _downsample_indices_stratified(
    *,
    ...
    """Downsample while roughly preserving (correct/error) x (congruent/incongruent) composition."""
...
def _scales_equivalent(a: Any, b: np.ndarray, *, atol: float = 1e-6) -> bool:
    """Stable comparison for persisted scale grids."""
    try:
...
def audit_baseline(*, output_root: Path, seed: int, subjects_per_group: int, test_fraction: float) -> None:
    ...
    manifest = {
        "schema_version": "true_single_subject_feasibility.v1",
        "created_at": _now_iso(),
        "seed": int(seed),
        ...
        "fit_contract": {
            "stage2_trainer": "train_age_groups_efficient.fit_stage2_from_logits",
...
def fit_age_group(
    *,
    ...
    for uid in subjects:
        ...
        split_row = splits_df.loc[
            (splits_df["age_group"] == age_group) & (splits_df["user_id"].astype(str) == uid)
        ]
        if split_row.empty:
        ...
        # Optional bounded trial budgets (keeps the feasibility workflow runnable).
        ...
        if existing_summary.exists() and existing_config.exists() and existing_params.exists():
            try:
                ...
                prior = {}
            # Only reuse cached results if they were produced by the current schema and match the
            # current run contract (epochs, temperature, scales, trial budgets, and exact split hash).
            ...
            if prior.get("schema_version") == "true_single_subject_feasibility.subject_eval.v1":
        ...
        subject_summary = {
            "schema_version": "true_single_subject_feasibility.subject_eval.v1",
            "age_group": age_group,
            "user_id": str(uid),
            "fit": {
